...
### v0.9 — Restant
1. ✅ **Fix actualités géopolitiques (GDELT)** (C8, 28/05/2026) : panneau réparé. Cause réelle = requêtes `OR` non parenthésées rejetées par GDELT (erreur texte en HTTP 200 → `response.json()` levait → `[]`). Corrigé : pattern ancre+groupe OR, tri pertinence, parsing défensif. Reste pour C8-bis : mots-clés enrichis par denrée, cache/fallback, timespan adaptatif.
