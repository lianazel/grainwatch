...
| Version | Date       | Résumé                                                                |
|---------|------------|-----------------------------------------------------------------------|
| 0.9.1   | 26/05/2026 | Refonte UX mobile (P1-P4) + correctifs post-test iPhone 14 (C1-C4) : sélecteur de source → sous-menu API + pastille `#sourcePastille`, garde-fou anti-débordement tooltips (`--tt-shift`), tooltip GrainTrack3D tactile, contraste dark mode (Sources + alerte déclenchée, WCAG AA), `setSource()` état unique barre↔menu |
...
### v0.9 — Restant
1. **Fix actualités géopolitiques (GDELT)** : le panneau ne charge jamais de données. Cause probable : l'appel GDELT ne filtre pas sur la céréale sélectionnée → passer le slug de la céréale en cours comme terme de recherche.
