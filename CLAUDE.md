...
|---------|------------|------------------------------------------------------------------|
| 0.9.1   | 26/05/2026 | Refonte UX mobile (P1-P4) + correctifs post-test iPhone 14 (C1-C4). Sélecteur de source déplacé de la barre vers un sous-menu API interactif dans le hamburger (Banque Mondiale/USDA + Simulation séparée par `<hr>` + GDELT lecture seule), pastille d'API active (`#sourcePastille`, ouvre le menu sur la section Sources) ; garde-fou anti-débordement viewport des tooltips (custom property `--tt-shift` dans les `transform`, flèche compensée) ; tooltip GrainTrack3D désactivé déclenché par **l'icône elle-même** au tap (C2 — le `title` natif ne s'affiche pas au tactile) ; contraste page Sources ET alerte déclenchée en dark mode (couleurs hardcodées → variables / override `[data-theme=dark]`, WCAG AA) ; sélecteur API masqué en portrait mobile durci `!important` (C4) ; icône Simulation `🔬→🧪`. `setSource()` = état unique synchronisant barre desktop ↔ menu mobile |
