...
### v0.9 — Restant
1. ✅ **Fix actualités géopolitiques (GDELT)** (C8→C9→C9-bis, 28/05/2026) — 3 couches de cause : (C8) requêtes `OR` non parenthésées rejetées en HTTP 200 texte → `response.json()` levait → `[]` ; (C9) GDELT bloqué CORS en prod → proxy serverless `api/gdelt.js` (`/api/gdelt`) ; (C9-bis) timeout 10 s insuffisant en prod → chaîne relevée à 25 s (proxy) / `maxDuration` 30 s / abort navigateur 30 s. **À valider en prod** (navigateur réel) : panneau peuplé sur Blé/Maïs/Riz, console sans erreur. Reste pour C8-bis : mots-clés enrichis par denrée, fallback en cascade, timespan adaptatif.
