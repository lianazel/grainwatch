...
### Fait — C9 : proxy Vercel Serverless pour GDELT (contournement CORS) (28/05/2026)
Rapport : `RAPPORT_CORRECTIF_C9_PROXY_GDELT_v1.md`. **Changement d'architecture** : 1ʳᵉ brique non statique du projet. C8 avait corrigé la syntaxe de requête mais le panneau restait vide **en prod** : GDELT n'envoie pas d'`Access-Control-Allow-Origin` → le navigateur bloquait tout `fetch` direct (CORS). La section n'a donc **jamais** fonctionné en prod.
- [x] **Création `api/gdelt.js`** (racine, hors `js/` et `site/`) — Vercel auto-détecte `api/` comme Serverless Function (Node 22, `fetch` natif, aucune dépendance, pas de `package.json` requis). Le front appelle `/api/gdelt` (même origine → pas de CORS), la fonction relaie côté serveur vers GDELT.
- [x] **Sécurité du proxy** : hôte/chemin GDELT **figés** (constante, pas de param client → pas de SSRF) ; whitelist stricte `['query','mode','maxrecords','format','timespan']` (autres params ignorés) ; `query` obligatoire et ≤ 500 car. ; défauts sûrs forcés (`format=json`, `mode=artlist`, `maxrecords=8`) ; aucun credential ; timeout 10 s (`AbortController`) ; parsing défensif `text()`+`JSON.parse` (GDELT renvoie ses erreurs en texte/HTTP 200) → 502 explicite ; cache Edge `Cache-Control: public, s-maxage=900, stale-while-revalidate=300` (absorbe le rate limit GDELT 1 req/5 s, zéro cache custom).
- [x] **Front `js/news.js:61`** : seule ligne changée — URL `https://api.gdeltproject.org/api/v2/doc/doc?…` → `/api/gdelt?…`. `encodeURIComponent(keywords)` déjà présent (`:58`). Parsing C8 et rendu inchangés (le proxy renvoie le même JSON).
- [x] **`vercel.json`** : inchangé. Il ne contient que des `headers` (`source:"/(.*)"`), **aucun rewrite/route** → ne shadow pas `/api/*`. La fonction tourne sans config. Les en-têtes CSP s'appliquent aussi à la réponse JSON (sans effet). CSP `connect-src` conserve `api.gdeltproject.org` (encore utilisé par le test direct de `sources.js`).
- [x] **Tests harnais local (Node, mock req/res)** : POST→405, query manquant→400, query>500→400, **param `evil` ignoré** (URL relayée ne contient que les params whitelistés + défauts forcés), 200+articles quand GDELT renvoie du JSON. Chemins d'erreur live validés : rate limit→502 (avec log non-JSON), timeout→504, fetch fail→502. `node --check` OK sur `api/gdelt.js` et `news.js`. La requête finale `wheat (price OR export …)` renvoyant 8 articles pertinents avait déjà été prouvée en C8 (curl).
- [x] **Synchro `site/`** : `js/news.js` recopié. `api/gdelt.js` **NON** copié (les serverless vivent à la racine ; GitHub Pages ne les supporte pas).
