...
Cycle de correctifs UX mobile (diagnostic `DIAGNOSTIC_MOBILE_v0.9.1.md`, problèmes P1 à P4) + ajustements après tests sur iPhone 14 (C1 à C4).
...
- **C1** — la pastille d'API active ouvre le menu **avec focus sur la section « Sources de données »** (raccourci ergonomique)
- **C2** — tooltip GrainTrack3D désactivé : **l'icône grisée elle-même déclenche la bulle** (le ⓘ séparé n'était pas remarqué) ; survol desktop + tap mobile, bénéficie du garde-fou `--tt-shift`
...
- **Message GrainTrack3D désactivé invisible au tap** sur tactile (cf. C2 ci-dessus)
...
- **C6** — toolbar mobile saturée (contrôles empilés, pastille par-dessus le logo) : cause racine identifiée — `.header-right { justify-content: flex-end }` faisait déborder le contenu vers la gauche en LTR, or `scrollWidth` ne mesure pas l'overflow côté start, donc la détection de débordement de `setupToolbarOverflow()` (`scrollWidth > clientWidth`) restait toujours fausse et aucun contrôle ne basculait dans le menu. Correctif : retrait de `justify-content: flex-end`, remplacé par un spacer `.header-right::before { flex: 1 }` (alignement à droite préservé, mais débordement vers la droite → `scrollWidth` fiable). Bonus : `.update-time` masquée en mobile pour libérer de l'espace. Aucun JS modifié. (Diagnostic `RAPPORT_DIAGNOSTIC_C6_TOOLBAR_MOBILE_v1.md`)
